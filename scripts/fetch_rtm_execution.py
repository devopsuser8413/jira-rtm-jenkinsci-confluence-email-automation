...
# -----------------------------
def safe_get(field, subfield=None, default="N/A"):
    value = fields.get(field)
    if isinstance(value, dict):
        return value.get(subfield, default) if subfield else value
...
# Extract data safely
# -----------------------------
project_key = safe_get("project", "key")
summary = safe_get("summary")
status = safe_get("status", "name")
issue_type = safe_get("issuetype", "name")
priority = safe_get("priority", "name")
reporter = safe_get("reporter", "displayName")
assignee = safe_get("assignee", "displayName", "Unassigned")
created_on = safe_get("created")
description = safe_get("description", "text", "No description provided.")
...
records = [
    ...
    {"Field": "Issue Key", "Value": jira_issue_key},
...
<p><b>Project:</b> {project_key} | <b>Type:</b> {issue_type}</p>
...
df_html = df.to_html(index=False, border=1, justify="center", classes="table table-striped")
...
print("🎉 Jira data fetched and reports generated successfully!")
